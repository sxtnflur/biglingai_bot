...
async def save_voice_as_mp3(voice: Voice, file_dir: str | None = None) -> str:
    """Скачивает голосовое сообщение и сохраняет в формате mp3."""
    file_dir = file_dir or f"{settings.BASE_DIR}/files"
    if not os.path.exists(file_dir):
        os.mkdir(file_dir)

    voice_mp3_path = f"{file_dir}/voice-{voice.file_unique_id}.mp3"
