...
from depends import payment_factory, logger_service
    ...
    try:
        async with async_session() as session:
            ...
            if not payment_method_id:
                scheduler.remove_job(get_job_id(user_id))
                ...
                await payment_factory.create_payment(
...
async def restart_autopayment():
    await logger_service.log_by_telegram_bot('Запускаю задачи на автооплаты')
    async with async_session() as session:
    ...
    await logger_service.log_by_telegram_bot('Задачи на автооплаты установлены')