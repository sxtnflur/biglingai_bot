...
'''.strip()
    ...
    async def choose_one_variant(
        ...
        print(f'{explanation=}')
        prompt = ((f'Группа ошибок: {group}\n' if group else '') +
                  f'Задание на отработку ошибки пользователя: {user_message}. '
                  f'Пояснение к ошибке: {explanation}')
        openai_service = OpenAIService(
            openai_client=self.openai,
            system_message=(
                'Ты - бот для русскоговорящих, изучающих английский язык. '
                'Придумывай для пользователя задание на знание английского языка с вариантами ответа '
                'на основании его ошибок. '
                'Задание должно быть именно на отработку той ошибки, которую сделал пользователь. '
    ...
    async def send_audio_talking(
        ...
        #     user_text = await OpenAIService(self.openai).transcript_audio(
